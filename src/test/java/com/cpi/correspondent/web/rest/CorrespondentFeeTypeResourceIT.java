...
/**
 * Integration tests for the {@Link CorrespondentFeeTypeResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CorrespondentFeeTypeResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CorrespondentFeeType createUpdatedEntity(EntityManager em) {
        CorrespondentFeeType correspondentFeeType = new CorrespondentFeeType()
    ...
    private void defaultCorrespondentFeeTypeShouldBeFound(String filter) throws Exception {
        restCorrespondentFeeTypeMockMvc.perform(get("/api/correspondent-fee-types?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(correspondentFeeType.getId().intValue())))
            .andExpect(jsonPath("$.[*].correspondentFeeTypeName").value(hasItem(DEFAULT_CORRESPONDENT_FEE_TYPE_NAME)))
            .andExpect(jsonPath("$.[*].sortNum").value(hasItem(DEFAULT_SORT_NUM)));

        // Check, that the count call also returns 1
        restCorrespondentFeeTypeMockMvc.perform(get("/api/correspondent-fee-types/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCorrespondentFeeType() throws Exception {
        ...
        CorrespondentFeeType updatedCorrespondentFeeType = correspondentFeeTypeRepository.findById(correspondentFeeType.getId()).get();
    ...
    public void deleteCorrespondentFeeType() throws Exception {
        ...
        // Delete the correspondentFeeType
