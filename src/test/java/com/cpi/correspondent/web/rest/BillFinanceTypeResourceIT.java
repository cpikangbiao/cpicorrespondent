...
/**
 * Integration tests for the {@Link BillFinanceTypeResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class BillFinanceTypeResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static BillFinanceType createUpdatedEntity(EntityManager em) {
        BillFinanceType billFinanceType = new BillFinanceType()
    ...
    private void defaultBillFinanceTypeShouldBeFound(String filter) throws Exception {
        restBillFinanceTypeMockMvc.perform(get("/api/bill-finance-types?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(billFinanceType.getId().intValue())))
            .andExpect(jsonPath("$.[*].billFinanceTypeName").value(hasItem(DEFAULT_BILL_FINANCE_TYPE_NAME)))
            .andExpect(jsonPath("$.[*].sortNum").value(hasItem(DEFAULT_SORT_NUM)));

        // Check, that the count call also returns 1
        restBillFinanceTypeMockMvc.perform(get("/api/bill-finance-types/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateBillFinanceType() throws Exception {
        ...
        BillFinanceType updatedBillFinanceType = billFinanceTypeRepository.findById(billFinanceType.getId()).get();
