...
/**
 * Integration tests for the {@Link CorrespondentFeeResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CorrespondentFeeResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CorrespondentFee createUpdatedEntity(EntityManager em) {
        CorrespondentFee correspondentFee = new CorrespondentFee()
    ...
    public void getAllCorrespondentFeesByCorrespondentFeeTypeIsEqualToSomething() throws Exception {
        // Initialize the database
        CorrespondentFeeType correspondentFeeType = CorrespondentFeeTypeResourceIT.createEntity(em);
    ...
    private void defaultCorrespondentFeeShouldBeFound(String filter) throws Exception {
        ...
        restCorrespondentFeeMockMvc.perform(get("/api/correspondent-fees/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCorrespondentFee() throws Exception {
        ...
        CorrespondentFee updatedCorrespondentFee = correspondentFeeRepository.findById(correspondentFee.getId()).get();
