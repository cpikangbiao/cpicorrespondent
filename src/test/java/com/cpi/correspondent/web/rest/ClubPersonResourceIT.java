...
/**
 * Integration tests for the {@Link ClubPersonResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class ClubPersonResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static ClubPerson createUpdatedEntity(EntityManager em) {
        ClubPerson clubPerson = new ClubPerson()
    ...
    private void defaultClubPersonShouldBeFound(String filter) throws Exception {
        restClubPersonMockMvc.perform(get("/api/club-people?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(clubPerson.getId().intValue())))
            .andExpect(jsonPath("$.[*].clubPersonName").value(hasItem(DEFAULT_CLUB_PERSON_NAME)))
        ...
        restClubPersonMockMvc.perform(get("/api/club-people/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateClubPerson() throws Exception {
        ...
        ClubPerson updatedClubPerson = clubPersonRepository.findById(clubPerson.getId()).get();
