...
/**
 * Integration tests for the {@Link CPICorrespondentResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CPICorrespondentResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CPICorrespondent createUpdatedEntity(EntityManager em) {
        CPICorrespondent cPICorrespondent = new CPICorrespondent()
    ...
    public void getAllCPICorrespondentsByCorrespondentTypeIsEqualToSomething() throws Exception {
        // Initialize the database
        CorrespondentType correspondentType = CorrespondentTypeResourceIT.createEntity(em);
    ...
    public void getAllCPICorrespondentsByClubPersonIsEqualToSomething() throws Exception {
        // Initialize the database
        ClubPerson clubPerson = ClubPersonResourceIT.createEntity(em);
    ...
    private void defaultCPICorrespondentShouldBeFound(String filter) throws Exception {
        restCPICorrespondentMockMvc.perform(get("/api/cpi-correspondents?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cPICorrespondent.getId().intValue())))
            .andExpect(jsonPath("$.[*].correspondentCode").value(hasItem(DEFAULT_CORRESPONDENT_CODE)))
            ...
            .andExpect(jsonPath("$.[*].keyWord").value(hasItem(DEFAULT_KEY_WORD)))
            .andExpect(jsonPath("$.[*].registerDate").value(hasItem(DEFAULT_REGISTER_DATE.toString())))
            .andExpect(jsonPath("$.[*].caseDate").value(hasItem(DEFAULT_CASE_DATE.toString())))
            .andExpect(jsonPath("$.[*].handlerUser").value(hasItem(DEFAULT_HANDLER_USER.intValue())))
            .andExpect(jsonPath("$.[*].remark").value(hasItem(DEFAULT_REMARK.toString())));
        ...
        restCPICorrespondentMockMvc.perform(get("/api/cpi-correspondents/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCPICorrespondent() throws Exception {
        ...
        CPICorrespondent updatedCPICorrespondent = cPICorrespondentRepository.findById(cPICorrespondent.getId()).get();
