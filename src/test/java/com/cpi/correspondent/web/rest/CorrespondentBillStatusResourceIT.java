...
/**
 * Integration tests for the {@Link CorrespondentBillStatusResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CorrespondentBillStatusResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CorrespondentBillStatus createUpdatedEntity(EntityManager em) {
        CorrespondentBillStatus correspondentBillStatus = new CorrespondentBillStatus()
    ...
    private void defaultCorrespondentBillStatusShouldBeFound(String filter) throws Exception {
        restCorrespondentBillStatusMockMvc.perform(get("/api/correspondent-bill-statuses?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(correspondentBillStatus.getId().intValue())))
            .andExpect(jsonPath("$.[*].correspondentBillStatusName").value(hasItem(DEFAULT_CORRESPONDENT_BILL_STATUS_NAME)))
            .andExpect(jsonPath("$.[*].sortNum").value(hasItem(DEFAULT_SORT_NUM)));

        // Check, that the count call also returns 1
        restCorrespondentBillStatusMockMvc.perform(get("/api/correspondent-bill-statuses/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCorrespondentBillStatus() throws Exception {
        ...
        CorrespondentBillStatus updatedCorrespondentBillStatus = correspondentBillStatusRepository.findById(correspondentBillStatus.getId()).get();
    ...
    public void deleteCorrespondentBillStatus() throws Exception {
        ...
        // Delete the correspondentBillStatus
