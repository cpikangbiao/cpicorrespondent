...
/**
 * Integration tests for the {@Link CorrespondentTypeResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CorrespondentTypeResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CorrespondentType createUpdatedEntity(EntityManager em) {
        CorrespondentType correspondentType = new CorrespondentType()
    ...
    private void defaultCorrespondentTypeShouldBeFound(String filter) throws Exception {
        restCorrespondentTypeMockMvc.perform(get("/api/correspondent-types?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(correspondentType.getId().intValue())))
            .andExpect(jsonPath("$.[*].correspondentTypeName").value(hasItem(DEFAULT_CORRESPONDENT_TYPE_NAME)))
            .andExpect(jsonPath("$.[*].sortNum").value(hasItem(DEFAULT_SORT_NUM)));

        // Check, that the count call also returns 1
        restCorrespondentTypeMockMvc.perform(get("/api/correspondent-types/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCorrespondentType() throws Exception {
        ...
        CorrespondentType updatedCorrespondentType = correspondentTypeRepository.findById(correspondentType.getId()).get();
