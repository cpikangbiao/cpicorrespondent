...
/**
 * Integration tests for the {@Link CorrespondentBillStatusLogResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpicorrespondentApp.class})
public class CorrespondentBillStatusLogResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CorrespondentBillStatusLog createUpdatedEntity(EntityManager em) {
        CorrespondentBillStatusLog correspondentBillStatusLog = new CorrespondentBillStatusLog()
    ...
    public void getAllCorrespondentBillStatusLogsByCorrespondentBillIsEqualToSomething() throws Exception {
        // Initialize the database
        CorrespondentBill correspondentBill = CorrespondentBillResourceIT.createEntity(em);
    ...
    private void defaultCorrespondentBillStatusLogShouldBeFound(String filter) throws Exception {
        restCorrespondentBillStatusLogMockMvc.perform(get("/api/correspondent-bill-status-logs?sort=id,desc&" + filter))
            ...
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(correspondentBillStatusLog.getId().intValue())))
            .andExpect(jsonPath("$.[*].billStatusName").value(hasItem(DEFAULT_BILL_STATUS_NAME)))
            .andExpect(jsonPath("$.[*].updateTime").value(hasItem(DEFAULT_UPDATE_TIME.toString())))
            .andExpect(jsonPath("$.[*].updateUser").value(hasItem(DEFAULT_UPDATE_USER.intValue())));

        // Check, that the count call also returns 1
        restCorrespondentBillStatusLogMockMvc.perform(get("/api/correspondent-bill-status-logs/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void updateCorrespondentBillStatusLog() throws Exception {
        ...
        CorrespondentBillStatusLog updatedCorrespondentBillStatusLog = correspondentBillStatusLogRepository.findById(correspondentBillStatusLog.getId()).get();
    ...
    public void deleteCorrespondentBillStatusLog() throws Exception {
        ...
        // Delete the correspondentBillStatusLog
